...
def _build_pi0_backbone_critic_defs(
    ...
    *,
    ...
    # OGPOSFTLearnerConfig inherits from AdvantageWeightedSFTLearnerConfig,
    # so this isinstance check covers both.
...
def _log_trainable_param_summary(agent: OGPOAgentLearner) -> None:
    """Sanity-check the freeze filter: report counts of trainable vs frozen
    parameters in the policy train state, so a startup glance confirms the
    PaliGemma backbone is frozen.
    """
    try:
        trainable = nnx.filter_state(
            agent._train_state.params, agent._config.trainable_filter
        )
        frozen = nnx.filter_state(
        ...
        if total == 0:
            ...
            return
        logging.info(
            "Trainable parameters: %d (%.2f%%) | Frozen: %d (%.2f%%) | Total: %d",
            trainable_count, 100.0 * trainable_count / total,
            frozen_count,    100.0 * frozen_count    / total,
            total,
        )
    except Exception as exc:  # pragma: no cover - diagnostic only
        logging.warning("Failed to log trainable param summary: %s", exc)
...
def main(config: _config.OnlineTrainConfig):
    ...
    if config.collect.store_prefix_rep:
        logging.info(
            "return_prefix_rep is enabled, but OGPO critics recompute prefix "
    ...
    _log_trainable_param_summary(agent)
    ...
    for step in pbar:
        ...
        if step % config.collect.eval_interval == 0:
            ...
            logging.info(
                f"Eval at step {step}: "
                + ", ".join(f"{k}={v:.4f}" for k, v in eval_info.items())
            )
