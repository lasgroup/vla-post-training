...
from src.rl.flow_awr.flow_awr_learner import FlowAWRLearner
...
def _build_pi0_backbone_critic_defs(
    ...
    *,
    ...
    assert isinstance(config.rl, _config.FlowAWRSFTLearnerConfig)
...
def main(config: _config.OnlineTrainConfig):
    ...
    if config.collect.store_prefix_rep:
        logging.info(
            "return_prefix_rep is enabled, but FlowAWR critics recompute prefix "
