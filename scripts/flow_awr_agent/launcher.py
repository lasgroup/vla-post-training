#!/usr/bin/env python3
"""Launcher for FlowAWR experiments.

FlowAWR = AWR critic (V/Q on buffer actions, exp(A/beta/scale) advantage
weighting) + per-flow-step PPO surrogate over the buffer-stored noise
trajectory. Bridges the gap between AWR (FM regression on buffer actions —
works) and awr_logprob/flow_mpo (log-prob loss on EMA samples — V baseline
mismatch).

Usage:
    ./scripts/flow_awr_agent/launcher.py --project_name flow_awr_sweep
    ./scripts/flow_awr_agent/launcher.py --project_name flow_awr_sweep --dry
"""
...
SCRIPT = "scripts/flow_awr_agent/exp.py"
CONFIG_NAME = "pi05_libero_online_flow_awr"
PROJECT_NAME = "flow_awr_sweep"
NUM_TRAIN_STEPS = 10_000

# AWR's tuned defaults are frozen; we sweep the FlowAWR-specific log-prob knobs
# (clip_epsilon, num_steps, noise_level) plus seed and task. online_ratio is
# pinned to 1.0 because offline data does not carry rollout_info.
applicable_configs: Dict[Union[str, tuple], List[Any]] = {
    ...
    "rl.num_steps": [10],
    "rl.noise_level": [0.3],
    # use_pessimistic_clip defaults to True (standard PPO `min`); add
    # `[True, False]` here to ablate the one-sided variant.
