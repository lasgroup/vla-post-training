...
#SBATCH --job-name=paper_ogpo_fast_s0
...
# derived from the 2026-09-20 probe series. Seed 0 of 3
...
#      full 100001-step run.
#
#      update_interval=100 IS THE VALUE HERE, not 1000. The paper excerpt's
#      prose ("<100 gradient steps... in single-task settings") was first
#      read as a target update COUNT, giving update_interval=1000 (~100
#      actor updates) -- confirmed working on L40S (job 10514270,
#      babel-m5-20: step 1000's actor update measured 39.7s). But the
#      ACTUAL parl_libero_tasks1_seed0_v0.yaml (marco/parl, this port's
#      real source) explicitly sets rl.policy.update_interval=100 as a
#      literal CLI value, which over num_train_steps=100000 gives ~1000
#      actor updates -- NOT <100. The maintainer's call (2026-09-20): treat
#      the paper prose's "<100" as the typo and the yaml's literal 100 as
#      authoritative. Training-loop projection at update_interval=100:
#      ~1000 actor updates x 39.7s + 100001 x 0.25s ~= 18h, vs ~5.6 days at
#      update_interval=10 -- still ~7.5x faster than the original recipe,
#      just ~2.25x slower than the update_interval=1000 version this
#      replaced.
#
...
#
# rl.n_samples=1 (not 8, and not the paperconfig family's 32) -- the
# maintainer's call 2026-09-20. n_samples only affects collect_data/
# evaluate_policy (sample_actions), never the training step, so it's
# orthogonal to everything above. At n_samples<=1, sample_actions falls
# through entirely to the single-sample base path
# (advantage_weighted_sft_learner.py:420) -- no best-of-N tiling, no
# critic-scoring pass on candidates at all, so this should be safer
# memory-wise than n_samples=8, not riskier. A separate rl.n_samples=1
# probe OOM'd earlier (job 10513755) but on a mixed-GPU-model card
# (confounded the same way the update_interval=1000 probe briefly was);
# not re-verified in isolation before folding in here.
#
...
echo "[paper-fast-s0] node=$(hostname) job=${SLURM_JOB_ID:-?} cuda=$CUDA_VISIBLE_DEVICES mujoco_egl=$MUJOCO_EGL_DEVICE_ID"
...
uv run scripts/exp.py pi05_libero_online_ogpo_ref \
  ...
  --exp_name stab_paper_expt_ogpo_single_task_fast_s0 \
