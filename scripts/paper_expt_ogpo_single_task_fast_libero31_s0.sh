...
#   - exp_name / group_name given a distinct libero31 tag so this does not
...
#
# Submit: sbatch scripts/paper_expt_ogpo_single_task_fast_libero31_s0.sh
# ---------------------------------------------------------------------------
set -uo pipefail
...
echo "[paper-fast-libero31-s0] node=$(hostname) job=${SLURM_JOB_ID:-?} cuda=$CUDA_VISIBLE_DEVICES mujoco_egl=$MUJOCO_EGL_DEVICE_ID"
...
uv run scripts/exp.py pi05_libero_online_ogpo_ref \
  ...
  --group_name policy_extraction_tasks1_ogpo_fast_libero31_v0 \
  --exp_name stab_paper_expt_ogpo_single_task_fast_libero31_s0 \
