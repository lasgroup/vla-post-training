...
#SBATCH --job-name=paper_ogpo_fast_libero38_s0
...
#
# Submit: sbatch scripts/paper_expt_ogpo_single_task_fast_libero38_s0.sh
# ---------------------------------------------------------------------------
set -uo pipefail
...
echo "[paper-fast-libero38-s0] node=$(hostname) job=${SLURM_JOB_ID:-?} cuda=$CUDA_VISIBLE_DEVICES mujoco_egl=$MUJOCO_EGL_DEVICE_ID"
...
uv run scripts/exp.py pi05_libero_online_ogpo_ref \
  ...
  --exp_name stab_paper_expt_ogpo_single_task_fast_libero38_s0 \
