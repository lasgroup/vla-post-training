#!/usr/bin/env python3
"""Launcher for MPO-weighted SFT experiments — sanity check between AWR and the
flow_* / awr_logprob variants.

The interesting knob here is `store_buffer_actions_in_batch`:

  True   — V trains on Q(s, a_buffer); actor weighted by Q(s, a_buf) - V(s).
           ALGORITHMICALLY IDENTICAL TO AWR. Should reproduce AWR's numbers.
  False  — V trains on Q(s, a_on_policy); actor weighted by
           Q(s, a_on_policy) - V(s). On-policy actor-critic variant.
           This is the plumbing flow_mpo / awr_logprob compose with.

Everything else is held at AWR's tuned values (beta=0.2, normalizer EMA=0.99,
use_mc_returns=False, lr=2.5e-5, ...). Three seeds, three tasks.

Usage:
    ./scripts/mpo_weighted_sft_agent/launcher.py --project_name mpo_wsft_sweep
    ./scripts/mpo_weighted_sft_agent/launcher.py --project_name mpo_wsft_sweep --dry
"""
...
SCRIPT = "scripts/mpo_weighted_sft_agent/exp.py"
CONFIG_NAME = "pi05_libero_online_mpo_sft"
PROJECT_NAME = "mpo_wsft_sweep"
NUM_TRAIN_STEPS = 10_000

# ---------- Hyperparameter grid ----------
# AWR's tuned defaults are frozen so the only difference between groups in
# wandb is `store_buffer_actions_in_batch`. The True column should track AWR;
# the False column tells you whether the on-policy critic+actor plumbing is
# healthy on its own (before flow_mpo / awr_logprob layer their losses on top).
applicable_configs: Dict[Union[str, tuple], List[Any]] = {
    ...
    # MPOWeighted-specific knobs.
    # The whole point of this sanity sweep: True ≈ AWR, False = on-policy variant.
    "rl.store_buffer_actions_in_batch": [True, False],
...
def main() -> None:
    ...
    for idx, combo in enumerate(combos):
        ...
        # Critic warmup ends right when policy training starts (matches AWR).
