...
from src.rl.flow_mpo.flow_mpo_learner import FlowMPOLearner
...
def _build_pi0_backbone_critic_defs(
    ...
    *,
    ...
    assert isinstance(config.rl, _config.FlowMPOSFTLearnerConfig)
...
def main(config: _config.OnlineTrainConfig):
    ...
    if config.collect.store_prefix_rep:
        logging.info(
            "return_prefix_rep is enabled, but Flow-MPO critics recompute prefix embeddings "
