...
#SBATCH --job-name=paper_ogpo_fast_libero31_s1
...
#
# Submit: sbatch scripts/paper_expt_ogpo_single_task_fast_libero31_s1.sh
# ---------------------------------------------------------------------------
set -uo pipefail
...
echo "[paper-fast-libero31-s1] node=$(hostname) job=${SLURM_JOB_ID:-?} cuda=$CUDA_VISIBLE_DEVICES mujoco_egl=$MUJOCO_EGL_DEVICE_ID"
...
uv run scripts/exp.py pi05_libero_online_ogpo_ref \
  ...
  --exp_name stab_paper_expt_ogpo_single_task_fast_libero31_s1 \
