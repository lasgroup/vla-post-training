...
from src.rl.fm_grpo.fm_grpo_learner import FMGRPOLearner
...
def _build_critic_defs(
    config: _config.OnlineTrainConfig,
    *,
    prefix_embedding_shape: tuple[int, ...] | None,
) -> tuple[StateActionCriticDef, StateValueDef]:
    assert isinstance(config.rl, _config.FMGRPOLearnerConfig)
...
def main(config: _config.OnlineTrainConfig):
    ...
    state_action_critic_def, state_value_def = _build_critic_defs(
        config, prefix_embedding_shape=prefix_embedding_shape
    )
    agent = FMGRPOLearner(
        config=config,
