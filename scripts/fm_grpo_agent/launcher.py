#!/usr/bin/env python3
"""Launcher for FM-GRPO experiments.

Usage:
    ./scripts/fm_grpo_agent/launcher.py --project_name my_project
    ./scripts/fm_grpo_agent/launcher.py --project_name my_project --dry
    ./scripts/fm_grpo_agent/launcher.py --project_name my_project --mode local
"""
...
SCRIPT = "scripts/fm_grpo_agent/exp.py"
CONFIG_NAME = "pi05_libero_online_fm_grpo"
PROJECT_NAME = "fm_grpo_sweep"
...
# ---------- Hyperparameter grid ----------
#
# group_size=1  → no group normalisation, degenerates to advantage-weighted FM loss.
# group_size>1  → GRPO: group-relative advantage normalisation within G samples.
#
# Memory: train_step expands B_reduced = batch_size // group_size observations to
# batch_size total samples for ONE FM forward pass (same footprint as AWR).
# sample_num_steps controls ODE quality for sampling — no gradient-memory cost.
applicable_configs: Dict[Union[str, tuple], List[Any]] = {
    ...
    # FM-GRPO fixed settings
    "rl.normalize_adv": [True],
    "rl.sample_num_steps": [10],   # ODE steps for sampling — no memory cost
    "rl.critic_inference_start_step": [0],
    # Best-of-N collection
    "collect.store_prefix_rep": [True],
    "rl.n_samples": [32],
    # Swept axes
    "rl.group_size": [1, 8, 16, 32],
    "rl.filtered_sft_weight": [0.0, 0.25],
