...
run_name="${RUN_NAME:-molmo_first30_sharded_$(date -u +%Y%m%d_%H%M%S)}"
output_dir="${OUTPUT_DIR:-${output_root%/}/${run_name}}"
output_prefix="${OUTPUT_PREFIX:-results}"
reset_output="${RESET_OUTPUT:-0}"
save_video_on_success="${SAVE_VIDEO_ON_SUCCESS:-0}"
...
echo "[runner] save_video_on_success=$save_video_on_success"
echo "[runner] video_dir=$video_dir"
...
for ((shard=0; shard<num_shards; shard++)); do
  ...
  (
    ...
    if [[ "$save_video_on_success" == "1" ]]; then
      cmd+=(--save-video-on-success)
    fi
