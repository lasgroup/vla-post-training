...
#SBATCH --job-name=paper_ogpo_fast_libero79_s0
#SBATCH --output=/home/pchellap/logs/%x_%j.out
#SBATCH --error=/home/pchellap/logs/%x_%j.out
# ---------------------------------------------------------------------------
# Paper experiment: OGPO, single train task libero_90_79, the FAST config,
# on maxlab (single RTX PRO 6000 / VRAM_96GB). Seed 0 of 2
# (paper_expt_ogpo_single_task_fast_libero79_s{0,1}.sh).
#
# Clone of paper_expt_ogpo_single_task_fast_s0.sh (the COMPLETED
# libero_90_82 campaign, general/L40S, fsdp_devices=2) with:
#   - task libero_90_82 -> libero_90_79 (--collect.tasks / --collect.eval_tasks)
...
#   - exp_name / group_name given a distinct libero79 tag so this does not
...
#
# Submit: sbatch scripts/paper_expt_ogpo_single_task_fast_libero79_s0.sh
# ---------------------------------------------------------------------------
set -uo pipefail
...
echo "[paper-fast-libero79-s0] node=$(hostname) job=${SLURM_JOB_ID:-?} cuda=$CUDA_VISIBLE_DEVICES mujoco_egl=$MUJOCO_EGL_DEVICE_ID"
...
uv run scripts/exp.py pi05_libero_online_ogpo_ref \
  ...
  --group_name policy_extraction_tasks1_ogpo_fast_libero79_v0 \
  --exp_name stab_paper_expt_ogpo_single_task_fast_libero79_s0 \
  ...
  --max_runtime 79200 \
  --collect.tasks libero_90_79 \
  --collect.eval_tasks libero_90_79 \
