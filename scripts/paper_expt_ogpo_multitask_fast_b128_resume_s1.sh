...
#SBATCH --job-name=paper_ogpo_mt4_fast_b128_s1
#SBATCH --gres=gpu:2
...
# ---------------------------------------------------------------------------
# RESUME of the multitask "fast" seed-1 run onto 2 GPUs (2026-09-21).
#
# Continues paper_expt_ogpo_multitask_fast_s1.sh (job 10514866, batch 32 /
# grad_accum 8 / FSDP=1, 1 GPU) from ITS OWN checkpoint: same ARM and SEED, so
# the same exp dir mt4_paper_expt_ogpo_multitask_fast_s1, and --resume is the
...
#
# Submit: sbatch scripts/paper_expt_ogpo_multitask_fast_b128_resume_s1.sh
# ---------------------------------------------------------------------------
set -euo pipefail
...
if [ "${DRY:-0}" != "1" ] && [ "${ALLOW_OVERLAP:-0}" != "1" ] && squeue -u "$USER" -h -n paper_ogpo_mt4_fast_s1 -t RUNNING,PENDING | grep -q .; then
  echo "[b128-resume-s1] REFUSING to start: batch-32 job paper_ogpo_mt4_fast_s1 is still queued/running and would race this job on the same checkpoint dir. Cancel it after its step-50000 checkpoint is finalized." >&2
  exit 1
fi
...
echo "[b128-resume-s1] node=$(hostname) job=${SLURM_JOB_ID:-?} arm=$ARM seed=$SEED batch=$BATCH fsdp=$FSDP gpu=$GPU"
echo "[b128-resume-s1] gpu_info=$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null | paste -sd';' || echo n/a)"
