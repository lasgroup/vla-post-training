...
#SBATCH --job-name=paper_ogpo_mt4_fast_b64a4_s0
#SBATCH --gres=gpu:1
...
# confounded the single-task probe series). Seed 0 of 3
...
echo "[paper-mt4-fast-b64a4-s0] node=$(hostname) job=${SLURM_JOB_ID:-?} arm=$ARM seed=$SEED"
echo "[paper-mt4-fast-b64a4-s0] gpu=$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null || echo n/a)"
