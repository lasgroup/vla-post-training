#!/usr/bin/env python3
"""Launcher for AWR-LogProb experiments.

Holds AWR's tuned defaults (beta=0.2, normalizer EMA=0.99, use_mc_returns=False)
constant and only sweeps the four knobs the new train_step adds:
clip_epsilon, num_steps, noise_level, use_ema_for_sampling. Three seeds.

Usage:
    ./scripts/awr_logprob_agent/launcher.py --project_name awr_logprob_sweep
    ./scripts/awr_logprob_agent/launcher.py --project_name awr_logprob_sweep --dry
    ./scripts/awr_logprob_agent/launcher.py --project_name awr_logprob_sweep --mode local
"""
...
SCRIPT = "scripts/awr_logprob_agent/exp.py"
CONFIG_NAME = "pi05_libero_online_awr_logprob"
PROJECT_NAME = "awr_logprob_sweep"
NUM_TRAIN_STEPS = 10_000

# ---------- Hyperparameter grid ----------
# Frozen at AWR's known-good values:
#   rl.beta=0.2, rl.normalizer_config.ema_weight=0.99, rl.use_mc_returns=False,
#   rl.policy_training_start_step=900, rl.online_ratio=1.0,
#   rl.reset_policy_params_to_ema_period=500, lr_schedule.value=2.5e-5,
#   rl.num_critic_updates_per_batch=10, td_weight_schedule=(0.5, 0.5, 500_000).
# Swept: clip_epsilon, num_steps, noise_level, use_ema_for_sampling, seed, task.
applicable_configs: Dict[Union[str, tuple], List[Any]] = {
    "seed": [0, 1, 2],
    "log_interval": [25],
    "batch_size": [256],

    # AWR's tuned defaults — keep frozen.
    "rl.beta": [0.2],
    ...
    # AWR-LogProb knobs — these are the ones we actually sweep.
    # use_pessimistic_clip defaults to True (standard PPO `min` form). Add
    # `[True, False]` here if you want to ablate against the one-sided variant.
...
def main() -> None:
    ...
    for idx, combo in enumerate(combos):
        ...
        # Keep critic warmup tied to policy start so the V/Q are warm before
        # advantages start driving the actor.
