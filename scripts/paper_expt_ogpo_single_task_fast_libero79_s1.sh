...
#SBATCH --job-name=paper_ogpo_fast_libero79_s1
...
# on maxlab (single RTX PRO 6000 / VRAM_96GB). Seed 1 of 2
...
#
# Submit: sbatch scripts/paper_expt_ogpo_single_task_fast_libero79_s1.sh
# ---------------------------------------------------------------------------
set -uo pipefail
...
echo "[paper-fast-libero79-s1] node=$(hostname) job=${SLURM_JOB_ID:-?} cuda=$CUDA_VISIBLE_DEVICES mujoco_egl=$MUJOCO_EGL_DEVICE_ID"
...
uv run scripts/exp.py pi05_libero_online_ogpo_ref \
  ...
  --exp_name stab_paper_expt_ogpo_single_task_fast_libero79_s1 \
