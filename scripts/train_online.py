...
from src.rl.filtered_sft_agent.filtered_sft_learner import get_env_and_agent_for_filtered_sft
    ...
    for step in pbar:
        ...
        if step % config.collect.collect_interval == 0:
            ...
            collect_info, n_collected_episodes = collect_data(
                agent=agent, env=env, config=config, task_description=task_description, step=step
            )
