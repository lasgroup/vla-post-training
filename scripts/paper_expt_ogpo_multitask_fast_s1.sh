...
#SBATCH --job-name=paper_ogpo_mt4_fast_s1
#SBATCH --gres=gpu:1
...
echo "[paper-mt4-fast-s1] node=$(hostname) job=${SLURM_JOB_ID:-?} arm=$ARM seed=$SEED"
echo "[paper-mt4-fast-s1] gpu=$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null || echo n/a)"
