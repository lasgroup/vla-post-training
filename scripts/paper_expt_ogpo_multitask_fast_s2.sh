...
#SBATCH --job-name=paper_ogpo_mt4_fast_s2
#SBATCH --gres=gpu:1
...
# confounded the single-task probe series). Seed 2 of 3
...
echo "[paper-mt4-fast-s2] node=$(hostname) job=${SLURM_JOB_ID:-?} arm=$ARM seed=$SEED"
echo "[paper-mt4-fast-s2] gpu=$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null || echo n/a)"
