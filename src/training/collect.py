...
from src.envs.libero import make_env_libero, init_state_libero, get_action_chunk_libero, get_frame_libero
...
def collect_data(config, checkpoint_path, data_path):
    ...
    for episode_idx in tqdm.tqdm(range(config.num_trials_per_task // config.env_num + 1)):

        logging.info(f"\nTask: {task_description}")
        action_plan = collections.deque()
        obs = init_state_libero(env, initial_states, episode_idx, config)
        frames, solved = [], [np.array([False] * config.env_num)]
        for _ in range(config.max_steps):

            if not action_plan:
                action_chunk = get_action_chunk_libero(obs, task_description, policy, config)
                action_plan.extend([action_chunk[:, i, :] for i in range(config.replan_steps)])
