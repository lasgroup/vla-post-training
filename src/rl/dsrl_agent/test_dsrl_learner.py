...
import src.rl.dsrl_agent.dsrl_learner as learner_mod
from src.rl.dsrl_agent.dsrl_learner_cfg import DSRLTrainConfig, CollectionConfig, SACModelConfig
...
def _make_learner_stub(*, num_envs=2, obs_prefix="pi0"):
    ...
    lrn = learner_mod.DSRLLearner.__new__(learner_mod.DSRLLearner)
    lrn._config = config
...
def test_sample_action_extracts_obs_prefix(monkeypatch):
    ...
    obs = {
        "observation/prefix_rep": np.array([[0.1, 0.2]], dtype=np.float32),
        "ignore_me": 123,
    }
...
def test_add_data_and_save_episode_inserts_stacked_episode(monkeypatch):
    ...
    def mk_step(t):
        obs = {
            "observation": {
                "prefix_rep": np.array([[t, t + 1], [t + 2, t + 3]], dtype=np.float32),
    ...
    assert inserted["observation"]["prefix_rep"].shape == (2,)
