...
class MPOLearner(MPOWeightedSFTLearner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert isinstance(rl_config, MPOLearnerConfig)
        ...
        def _policy_wrapper(batch, policy_state, q_state, value_state, rng, alpha_kl):
        ...
        self._update_policy_jitted = jax.jit(
            ...
            in_shardings=(
                ...
                self._replicated_sharding,  # alpha_kl
            ),
            out_shardings=(
                self._train_state_sharding,
    ...
    def _update_policy(
        ...
        **kwargs,
        ...
        if mc_return is not None:
            raise ValueError(
                "MPOLearner does not support use_mc_returns=True. "
                "Advantages are computed internally from Q-V."
            )
        # MPO train_step samples its own actions internally, so the MPO on-policy action is wasted.
    ...
    def update(self) -> dict:
        assert isinstance(self._config.rl, MPOLearnerConfig)
        rl_config = self._config.rl
        ...
        if use_online:
            ...
            if (
                ...
                self._reserve_buffer.insert(
                    jax.tree.map(np.asarray, online_batch),
                )
        ...
        if update_policy:
            ...
            if batch_size % group_size != 0:
                raise ValueError(
                    "MPO actor batch size must be divisible by the "
            ...
            # Adaptive M-step KL: update Lagrangian multiplier (paper Eq. 12).
            if rl_config.use_adaptive_kl:
                kl_div = actor_info.get(
                    "kl_divergence", jnp.asarray(0.0, dtype=jnp.float32)
                )
                self._alpha_kl = jnp.maximum(
                    jnp.float32(0.0),
                    self._alpha_kl
                    + jnp.float32(rl_config.alpha_kl_lr)
                    * (jnp.asarray(kl_div, dtype=jnp.float32) - jnp.float32(rl_config.epsilon_m)),
                )
                actor_info["alpha_kl"] = self._alpha_kl
