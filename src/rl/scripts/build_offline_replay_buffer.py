#!/usr/bin/env python3
"""Convert a LeRobot dataset into a ShardedReplayBuffer saved to disk.

Run once to pre-compute the offline buffer, then point the learner at the
output directory via ``offline_buffer_load_paths``.
"""
...
def _episode_to_online_payload(
    ...
    *,
    ...
    _actions = np.stack(
        [actions[start : start + action_horizon] for start in range(n_windows)], axis=0
    )
    ...
    _discount = np.asarray(
        [
            0.0 if np.any(done[start : start + action_horizon]) else last_gamma
            for start in range(n_windows)
        ],
        dtype=np.float32,
    )
    _mc_return = (
        (all_gammas * rewards[:n_steps])[::-1].cumsum()[::-1] / all_gammas
    )[:n_windows].astype(np.float32)
    ...
    def transform(obs_dict: dict[str, Any], act: np.ndarray, text_prompt: str):
        ...
        proc["image_mask"] = {
            k: np.full((n_windows,), bool(v)) for k, v in proc["image_mask"].items()
        }
...
def run(args):
    ...
    for idx in range(len(ds)):
        frame = ds[idx]
        ep_id = int(frame["episode_index"].item())
