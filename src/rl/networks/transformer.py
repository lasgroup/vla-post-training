...
class _TransformerBlock(nnx.Module):
    """Pre-RMSNorm transformer block with bias-free SwiGLU FFN."""
...
class Transformer(nnx.Module):
    """Modern Pre-RMSNorm transformer encoder (Llama-style block).

    Drop-in alternative to `MLP` for the best-of-N critic encoder. Each entry of
    `hidden_dims` is the d_model of one transformer block; `len(hidden_dims)` is
    the number of blocks. The output is the token-axis mean-pooled embedding of
    size `hidden_dims[-1]`.

    Accepts either a `(B, T, D)` sequence or a `(B, D)` flat input (unsqueezed
    to a single token).
    """
    ...
    def __init__(
        ...
        *,
        ...
        assert len(hidden_dims) > 0, "hidden_dims must be non-empty"
