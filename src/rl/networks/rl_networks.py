...
from src.rl.networks.decoders.values.state_action_value import StateActionValueDecoder, StateActionEnsembleDecoder
from src.rl.networks.decoders.values.state_value import StateValueDecoder, StateValueEnsembleDecoder
from src.rl.networks.decoders.policies.normal_policy import NormalPolicyDecoder
from src.rl.networks.decoders.policies.learned_std_normal_policy import LearnedStdNormalPolicyDecoder, \
...
class StateActionCritic(nn.Module):
    ...
    def __call__(self, observation: ObsType, action: ActionType, training: bool = False) -> jnp.ndarray:
        ...
        q = self.state_action_decoder(observations=embedding, actions=action, training=training)
        return q
