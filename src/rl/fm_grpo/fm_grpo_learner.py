...
class FMGRPOLearner(MPOWeightedSFTLearner):
    ...
    def update(self) -> dict:
        ...
        # Critics are small MLPs; a larger dedicated batch improves TD stability.
        critic_batch_size = self._config.rl.critic.batch_size or online_batch_size
        ...
        is_success = None
        if use_online:
            # Sample policy and critic batches independently so that the critic
            # can use a larger batch size without affecting the policy sample.
            critic_online_batch = (
                self._online_data_buffer.sample(batch_size=critic_batch_size)
                if update_critic else None
            )
