...
Closely mirrors DSRLLearner but operates on residual actions. The SAC
actor/critic receive ``base_action`` concatenated into the state vector so
they can condition on what the base policy proposes.
"""
...
# ---------------------------------------------------------------------------
# Image helpers (shared with DSRL)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Observation extraction — extends DSRL version to handle base_action
# ---------------------------------------------------------------------------
...
def _extract_replay_observation(
    ...
    *,
    ...
    - optional ``base_action`` (residual RL specific)
    """
    ...
    # Residual RL: preserve base_action from the observation.
...
def _finalize_replay_observation(
    ...
    if "base_action" in next_obs or "base_action" in current_obs:
        merged["base_action"] = np.asarray(
            next_obs.get("base_action", current_obs.get("base_action")), dtype=np.float32
        )
    return merged
...
def _normalize_residual_observation(observations: Any) -> Any:
    ...
    Like normalize_observation_for_model but also handles ``base_action``:
    the base action is flattened and concatenated into the state vector so
    the SAC actor/critic can condition on it.
    """
    normalized = normalize_observation_for_model(observations)
    if not isinstance(normalized, dict):
        return normalized

    # If base_action is present, concatenate it into the state vector.
...
def _normalize_replay_observation_for_model(obs_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    so we just concatenate and convert to float32.
    """
...
class ResidualRLLearner(Agent):
    ...
    def __init__(
        ...
        # Normalize dummy observation for network init — concatenates base_action into state.
        dummy_obs_normalized = _normalize_residual_observation(raw_dummy_obs)
        ...
        ckpt_dir = epath.Path(self._config.checkpoint_dir) / "residual_rl_state"
        ...
        logger.info(
            "ResidualRLLearner init: action_dim=%d, sac_image_size=%d, crop_padding=%d, "
            "target_entropy=%.3f, warmup_noise=%s, residual_scale=%.3f",
            self._action_dim, self._sac_image_size, self._crop_padding,
            self._target_entropy, self._use_random_noise, self._residual_action_scale,
        )
    ...
    def _sample_action(
        ...
        *,
        ...
        """Sample residual actions conditioned on the current observation + base_action."""
        processed_obs = _normalize_residual_observation(observations)
        ...
        actions = actions * self._residual_action_scale
    ...
    # ------------------------------------------------------------------
    # Image augmentation (identical to DSRL)
    # ------------------------------------------------------------------
    ...
    def update(self):
        ...
        if self.training_steps % int(getattr(self._config.rl, "critic_update_frequency", 1)) == 0:
            ...
            batch_observation = _normalize_replay_observation_for_model(batch["observation"])
            batch_next_observation = _normalize_replay_observation_for_model(batch["next_observation"])
    ...
    # ------------------------------------------------------------------
    # Episode saving with sparse -1/0 reward (matching DSRL)
    # ------------------------------------------------------------------
    ...
    def save_episode(self, is_success: bool = False, env_index: int = 0, **kwargs):
        ...
        Uses the same sparse reward scheme as DSRL:
        ...
        for idx, ep in enumerate(episode):
            ...
            # Store the residual actions in replay.
    ...
    def _restore_checkpoint(self):
        if not self._ckpt_dir.exists():
            logger.info("No residual RL checkpoint directory found, starting from scratch.")
            return
        ...
        if not steps:
            logger.info("No residual RL checkpoints found, starting from scratch.")
            return
        ...
        logger.info("Restored residual RL checkpoint from step %d", step)
