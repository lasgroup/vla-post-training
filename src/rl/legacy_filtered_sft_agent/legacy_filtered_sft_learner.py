...
def init_train_state(
        config: OnlineTrainConfig, init_rng: at.KeyArrayLike, mesh: jax.sharding.Mesh, *, resume: bool
...
class LegacyFilteredSFTLearner(Agent):
    def __init__(self, config: OnlineTrainConfig):
        ...
        self._data_sharding = jax.sharding.NamedSharding(self._mesh, jax.sharding.PartitionSpec(sharding.DATA_AXIS))
        ...
        self._policy_sharding_spec = jax.sharding.NamedSharding(
            jax.sharding.Mesh(
                jax.sharding.mesh_utils.create_device_mesh((len(jax.devices()),)), axis_names=('batch',)),
                jax.sharding.PartitionSpec('batch', )
        )

        # initialize checkopointing, wandb
        self._checkpoint_manager, self._resuming = _checkpoints.initialize_checkpoint_dir(
        ...
        self._data_loader = create_data_loader(config, sharding=self._data_sharding, shuffle=True)
        ...
        self._train_state, self._train_state_sharding = init_train_state(self._config, init_rng, self._mesh,
        ...
        self._train_step = jax.jit(
            ...
            in_shardings=(self._replicated_sharding, self._train_state_sharding, self._data_sharding),
    ...
    def _setup_lerobot_dataset(self, step: int | None = None):
        ...
        self._lerobot_dataset = lerobot_dataset.LeRobotDataset.create(
            ...
            robot_type="panda",
            fps=10,
            features={k: v for k, v in lerobot_dataset.LeRobotDatasetMetadata(self._config.data.repo_id).features.items() if
                      k in allowed_keys},
            image_writer_threads=10,
            image_writer_processes=5,
        )

    def _process_obs_for_pi0(self,
                             observations: Dict,
                             task_description: str | None = None,
                             ):
        # If we are stacking all the observations in the
        if self._config.collect.add_per_step_data:
            current_obs = jax.tree_util.tree_map(lambda x: x[:, -1], observations["observation"])
        else:
                ...
                if obs_key == "prompt":
                    ...
                    if 'image' in obs_key and self._config.collect.resize_image > 0:
                        ...
                        val = image_tools.convert_to_uint8(
                            image_tools.resize_with_pad(val,
    ...
    def eval_actions(self,
                     observations: np.ndarray | Dict, **kwargs) -> np.ndarray:
        task_description = kwargs.get("task_description")
        batch_actions = kwargs.get("batch_actions")
        if batch_actions is None:
            batch_actions = False
        rng, self._rng = jax.random.split(self._rng)
        processed_obs = self._process_obs_for_pi0(observations, task_description=task_description)
    ...
    def save_checkpoint(self, step: int | None = None):
        if step is None:
            step = self.training_steps
        _checkpoints.save_state(self._checkpoint_manager, self._train_state, self._data_loader, step)
    ...
    def add_data(self, step_data: StepData):
        ...
        for i in range(self._config.collect.env_num):
            self._episode_storage[i].append(
                get_env_value(step_data, i)
            )
        ...
        if self._config.collect.add_per_step_data:
            ...
            for n_frame, ep in enumerate(episode_data):
                ...
                for step in range(total_chunks):
                    ...
                    self._lerobot_dataset.add_frame(process_frame(obs), task=str(task_description))
        else:
    ...
    def end_data_collection(self, step: int | None = None):
        ...
        self._data_loader = create_data_loader(self._config, sharding=self._data_sharding, shuffle=True,
