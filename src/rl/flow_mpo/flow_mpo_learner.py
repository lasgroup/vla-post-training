...
AWR-style exp(A/beta/scale) weighting + PPO-clipped ratio on EMA-sampled flow rollouts 
+ optional adaptive (DreamerV3-style) advantage scale. 

This is NOT the original MPO algorithm, see `src/rl/mpo/mpo.py` for the actual MPO learner.
"""
import gc
import functools
...
class FlowMPOLearner(MPOWeightedSFTLearner):
    ...
    def _get_on_policy_action(
        ...
        assert isinstance(rl_config, FlowMPOSFTLearnerConfig)
