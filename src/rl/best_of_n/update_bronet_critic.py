...
def train_bronet_q_step(
    ...
    target_v = _bro_pessimistic_reduce(next_v, config.rl.bronet_pessimism)     # (B,)

    def loss_fn(q_model, observation, actions):
        td_weight = jnp.clip(config.rl.td_weight_schedule.create()(step), 0.0, 1.0)
...
def train_bronet_value_step(
    ...
    q_values = q_model(observation, actions)                                   # (num_qs, B)
    target_q = _bro_pessimistic_reduce(q_values, config.rl.bronet_pessimism)  # (B,)
