...
from src.rl.residual_rl_agent.residual_rl_learner_cfg import ResidualRLTrainConfig, SACModelConfig
...
def get_env_and_agent_for_residual_rl(env_fn, config, task_description, env_class):
    env = residual_rl_wrap_env(
        env_fn=env_fn,
        config=config,
        task_description=task_description,
        env_class=env_class,
    )
    agent = ResidualRLLearner(config)
    return env, agent

def residual_rl_wrap_env(env_fn: EnvFn, config: ResidualRLTrainConfig, task_description: str, env_class: str):
    ...
    env = (
        ResidualRLVectorEnv(env_factories)
        if env_num > 1
...
class ResidualRLLearner(Agent):
    def __init__(self, config: ResidualRLTrainConfig):
        self._config = config
    ...
    def _get_online_replay_buffer(
        ...
        dummy_obs_dict = {
            "base_action": np.zeros((1, obs_dim), dtype=np.float32),
        }
        ...
        def _preprocess_insert(episode_data: Dict[str, Any]):
            base_action = np.asarray(episode_data["observation"]["base_action"], dtype=np.float32)
            actions = np.asarray(episode_data["actions"], dtype=np.float32)
            actions_flat = actions.reshape(actions.shape[0], -1)
            next_base_action = np.asarray(episode_data["next_observation"]["base_action"], dtype=np.float32)
            ...
            return {
                "observation": {"base_action": base_action},
                "actions": actions_flat,
                "next_observation": {"base_action": next_base_action},
    ...
    def _sample_action(
        ...
        obs_prefix = observations.get("observation/base_action")
        if obs_prefix is None:
            obs_prefix = observations.get("base_action")
    ...
    def save_episode(self, is_success: bool = False, env_index: int = 0, **kwargs):
        ...
        def process_frame(
            ...
            *,
            ...
            obs_dict = ob if "base_action" in ob else ob["observation"]
            base_action = obs_dict["base_action"]
            frame = {"base_action": np.asarray(base_action, dtype=np.float32)}

            frame["actions"] = np.asarray(actions, dtype=np.float32)
            next_state = frame["base_action"]
            if next_ob is not None:
                next_obs_dict = next_ob if "base_action" in next_ob else next_ob["observation"]
                if "base_action" in next_obs_dict:
                    next_state = next_obs_dict["base_action"]
                
            frame["next_observation"] = {"base_action": np.asarray(next_state, dtype=np.float32)}
        ...
        if self._config.collect.add_per_step_data:
            ...
            episode_batch_formatted = {
                "observation": {"base_action": episode_batch["base_action"]},
                "actions": episode_batch["actions"],
                "next_observation": {"base_action": episode_batch["next_observation"]["base_action"]},
